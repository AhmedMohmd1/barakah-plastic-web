...
**Styling**: brand colors are hardcoded hex in [tailwind.config.ts](tailwind.config.ts) (`primary` #003049 deep blue, `secondary` #F77F00 orange, `accent` gold) while the rest of the shadcn palette uses HSL CSS variables from `src/index.css`. Shared section classes like `section-padding`, `container-custom`, `heading-2`, and scroll animations (`scroll-animate` + `useScrollAnimation` hook) are used across all landing sections.
